...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (newStatus) {
    // Унифицированная модель — одно и то же значение в обоих полях
    await db`UPDATE asystem_requests SET lead_status = ${newStatus}, status = ${newStatus} WHERE request_id = ${id}`;
  }
  if (newNote !== null) {
